...
// Get list project that user join taht complete
app.get('/project/join', authenticateToken, async (req, res) => {
  try {
    connection.query(
      `SELECT * FROM project INNER JOIN user_project ON project.project_id = user_project.user_project_group_id WHERE user_project_user_id = ${req.user.user_id} AND project_status != 0`,
      (err, result, fields) => {
        if (err) {
          console.log('Error in the query', err);
          return res.status(400).send();
...
app.get('/patient', authenticateToken, async (req, res) => {
  try {
    connection.query(`SELECT * FROM patient`, (err, result, fields) => {
      if (err) {
        console.log('Error in the query', err);
        return res.status(400).send();
      }
...
app.get(
  '/patient/:project_id/project/check',
  authenticateToken,
  async (req, res) => {
    const { project_id } = req.params;
    try {
...
// Get list project that user join taht complete
app.get('/project/join/:user_id', authenticateToken, async (req, res) => {
  const { user_id } = req.params;
  try {
    connection.query(
      `SELECT * FROM project INNER JOIN user_project ON project.project_id = user_project.user_project_group_id WHERE user_project_user_id = ${user_id} AND project_status != 0`,
      (err, result, fields) => {
        if (err) {
          console.log('Error in the query', err);
          return res.status(400).send();
